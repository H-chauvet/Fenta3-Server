...
namespace server
{
    ...
    class TCPGameServer
    {
        ...
        public bool IsPlayerInServer(TcpMessageChannel pClient)
        {
            if (_playerInfo.ContainsKey(pClient)) return true;
            return false;
        }
        ...
        /**
		 *         * Should be called by the LoginRoom when a player joins.
		 *                 */
        public void AddPlayerInfo(TcpMessageChannel pClient, PlayerInfo pInfo)
        {
            if (_playerInfo.ContainsKey(pClient)) return;
            _playerInfo.Add(pClient, pInfo);
        }
        public PlayerJoinResponse.RequestResult AddPlayerToServer(TcpMessageChannel pClient, PlayerInfo pInfo)
        {
            if (_playerInfo.ContainsKey(pClient)) return PlayerJoinResponse.RequestResult.NOTUNIQUE;
            _playerInfo.Add(pClient, pInfo);
